        ...
        <form action="@if(empty($order)) {{ url("purchaseorders") }} @else {{ route('purchaseorders.update',['codigo'=>str_pad($order->id,10,'0',STR_PAD_LEFT)]) }} @endif" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="date" class="form-control form-control-sm" id="fecha_emision" name ="fecha_emision" placeholder="dd/mm/aaaa" @unless(empty($order)) value="{{ $order->fecha_emision }}" @else value="{{ old('fecha_emision') }}" @endunless/>
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" class="form-control form-control-sm " disabled id="proveedor" name="proveedor" placeholder="Proveedor" @unless(empty($order)) value="{{ $order->ruc }} - {{ $order->razon_social }}" @else value="{{ old('proveedor') }}" @endunless aria-describedby="btn_buscar"/>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <div class="input-group">
                        ...
                        @else
                            <input type="text" class="form-control form-control-sm " value="{{ $order->descor }}" />
                        @endunless
                    </div>
                </div>
                <div class="col-md-4 mb-3">
                        ...
                        @unless(empty($forma_pago))
                            <select name="condicion_pago" id="condicion_pago" class="form-control form-control-sm">
                                ...
                                @foreach ($forma_pago as $forma)
                                    <option value="{{ $forma->codtab }}" @unless(empty($order)) @if($order->condicion_pago == $forma->codtab ) selected @endif @else @if(old('destino') == $forma->codtab ) selected @endif @endif>{{ $forma->descor }}</option>    
                                @endforeach
                            </select>
                        @else
                            <input type="text" class="form-control form-control-sm " value="{{ $order->condicion_pago }}" />    
                        @endunless
                    </div>
                </div>
                <div class="col-md-4 mb-3">
            ...
            <div class="row align-items-end ">
                ...
                <div class="col-md-1 mb-3">
                    <div class="input-group">
                        <button type="button" @if(empty($order)) disabled @endif class="btn btn-sm btn-primary" id="btn_detalle">Detalle</button>
                    </div>
                </div>
            </div>
            <h1></h1>
            ...
            @if ($activo)
                <div class="btn-toolbar mb-2 mb-md-0">
                    <div class="btn-group mr-2">
                        ...
                        <a href="{{ route('purchaseorders') }}" class="btn btn-sm btn-primary border">Salir</a>
                    </div>
                </div>    
            @endif
    ...
    function procesar_rpta(rpta){
        ...
        if(rpta.success){
            $("#tabla_detail tbody" ).append(
                ...
                '<td>'+$('#total').val()+'</td>'+
                '<td> <a class="sel badge badge-primary" href="#" id_item = "'+rpta.id+'" val_po="'+$('#purchase_order_id').val()+'" >Eliminar</a></td>'+
                '</tr>');
        }   
        else{
