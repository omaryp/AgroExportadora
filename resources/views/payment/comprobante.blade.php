<div class="row">
    ...
    <div class="col-md-4 mb-3">
        ...
        <div class="input-group">
            @unless(empty($cuotas))
                <select name="nro_cuota" id="nro_cuota" class="form-control form-control-sm" >
                    <option value="00">Seleccionar Cuota</option>
                    @foreach ($cuotas as $parm)
                        <option value="{{ $parm->nro_cuota }}" @unless(empty($pago)) @if( $pago->nro_cuota == $parm->cuota ) selected @endif @endif > {{ $parm->nro_cuota }}</option>    
                    @endforeach
                </select>
            @endunless
        </div>
...
<div class="row">
    <div class="col-md-4 mb-3">
        ...
        <div class="input-group"> 
            @unless(empty($bancos))
                <select name="codigo_banco" id="codigo_banco" class="form-control form-control-sm" >
        ...
        @unless(empty($medios))
            <select name="medio_pago" id="medio_pago" class="form-control form-control-sm" >
                ...
                @foreach ($medios as $parm)
                    <option value="{{ $parm->codtab }}" @unless(empty($pago)) @if( $pago->medio_pago == $parm->codtab ) selected @endif @else @if(old('medio_pago') == $parm->codtab ) selected @endif @endif > {{ $parm->descor }}</option>    
                @endforeach
            </select>
        @endunless    
    </div>
    <div class="col-md-4 mb-3">
        ...
        <input type="text" class="form-control form-control-sm"  id="nro_doc_pago" name="nro_doc_pago" placeholder="Documento Pago" @unless(empty($pago)) value="{{ $pago->nro_doc_pago }}" @else value="{{ old('nro_doc_pago') }}" @endunless aria-describedby="btn_buscar"/>    
    </div>
</div>
...
<div class="row">
    <div class="col-md-12 mb-3">
        ...
        <input type="text" class="form-control form-control-sm"  id="glosa" name="glosa" placeholder="Glosa" @unless(empty($pago)) value="{{ $pago->glosa }}" @else value="{{ old('glosa') }}" @endunless aria-describedby="btn_buscar"/>    
    </div>
</div>